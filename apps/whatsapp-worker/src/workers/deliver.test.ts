...
describe("deliverMessage", () => {
  it("sends text messages as text and never touches storage", async () => {
    ...
    await expect(deliverMessage(p, media, text, timeouts)).resolves.toEqual({ providerMessageId: "text-1" });
  ...
  it("sends a video with the body as caption", async () => {
    ...
    await expect(deliverMessage(p, media, video, timeouts)).resolves.toEqual({ providerMessageId: "video-1" });
