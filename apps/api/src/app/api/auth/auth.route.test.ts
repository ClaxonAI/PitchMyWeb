...
import { createOrder, verifyPayment } from "../../../lib/checkout/checkout.service";
...
const FAKE_KEY_SECRET = "test_secret";
const fakeRazorpay: RazorpayClient = { createOrder: async ({ amount, currency }) => ({ id: `rzp_order_${Math.random().toString(36).slice(2)}`, amount, currency }) };

async function createPaidOrder() {
  const order = await createOrder(prisma, fakeRazorpay, { planId: "auto", market: "india" });
  createdOrderIds.push(order.orderId);
  const signature = computeRazorpaySignature(FAKE_KEY_SECRET, order.razorpayOrderId, "pay_123");
  await verifyPayment(prisma, FAKE_KEY_SECRET, { orderId: order.orderId, razorpayPaymentId: "pay_123", razorpaySignature: signature });
...
describe("handleRegister", () => {
  ...
  it("attaches a PAID, unclaimed order (from a post-payment redirect) to the new account", async () => {
    const orderId = await createPaidOrder();
    const email = uniqueEmail("register-with-order");
    const response = await handleRegister(prisma, jsonRequest("http://localhost/api/auth/register", { email, password: "correcthorsebattery", orderId }));
    createdUserIds.push((await response.json()).id);

    const stored = await prisma.order.findUnique({ where: { id: orderId } });
    expect(stored?.userId).toBe((await prisma.user.findUnique({ where: { email } }))?.id);
  });

  it("does not fail registration when orderId points at an order that isn't PAID", async () => {
    const razorpay: RazorpayClient = { createOrder: async ({ amount, currency }) => ({ id: `rzp_order_${Math.random().toString(36).slice(2)}`, amount, currency }) };
    const unpaid = await createOrder(prisma, razorpay, { planId: "auto", market: "india" });
    createdOrderIds.push(unpaid.orderId);

    const email = uniqueEmail("register-with-unpaid-order");
    const response = await handleRegister(prisma, jsonRequest("http://localhost/api/auth/register", { email, password: "correcthorsebattery", orderId: unpaid.orderId }));
    expect(response.status).toBe(201);
    createdUserIds.push((await response.json()).id);

    const stored = await prisma.order.findUnique({ where: { id: unpaid.orderId } });
...
describe("handleLogin", () => {
  ...
  it("rejects a suspended account after a correct password", async () => {
    const email = uniqueEmail("login-suspended");
    const registerResponse = await handleRegister(prisma, jsonRequest("http://localhost/api/auth/register", { email, password: "correcthorsebattery" }));
    const userId = (await registerResponse.json()).id;
    createdUserIds.push(userId);
    await prisma.user.update({ where: { id: userId }, data: { suspendedAt: new Date() } });

    await expect(handleLogin(prisma, jsonRequest("http://localhost/api/auth/login", { email, password: "correcthorsebattery" }))).rejects.toThrow(AccountSuspendedError);
  });
