...
async function newAnalyzableLead(prefix: string) {
  ...
  const providerInput: BusinessProviderInput = {
    ...
    // tier-4/tier-2 match another file's identically-fixtured business.
  ...
  it("rejects the 21st analyze call within the window for the same user", async () => {
    ...
    for (let i = 0; i < 20; i += 1) {
      ...
      const { lead } = await ingestBusinessAsLead(prisma, {
        ...
        providerInput: {
          ...
          address: "12 MG Road",
          city: "Chennai",
          // Phase 2 dedup engine: distinct per iteration so these 20
          // businesses aren't collapsed into one by tier 2 (phone+city).
          phone: uniqueIndianPhone(),
          email: null,
          website: null,
          instagram: null,
    ...
    const { lead: overLimitLead } = await ingestBusinessAsLead(prisma, {
      ...
      providerInput: {
        ...
        address: "12 MG Road",
        city: "Chennai",
        // Phase 2 dedup engine: distinct from the loop above's businesses.
        phone: uniqueIndianPhone(),
        email: null,
        website: null,
        instagram: null,
