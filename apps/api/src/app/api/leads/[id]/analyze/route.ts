...
// scoring -> AI client -> Zod validation -> business validation ->
// persistence -> activity -> lifecycle transition, all inside
// analyzeLead). No AI/business logic lives in this file.
//
// `ai` is injectable (defaults to the real env-configured client) so
// tests can pass a fake AiClient without ever touching a real model
...
// caller should get.
export async function handleAnalyzeLead(db: PrismaClient, request: NextRequest, params: { id: string }, ai?: AiClient): Promise<NextResponse> {
  ...
  // checked before the expensive model call, same pattern as login's
  // rate limit (lib/api/rate-limit.ts).
  await rateLimit(`analyze:user:${user.id}`, 20, WINDOW_MS);

  await analyzeLead(db, ai ?? createAiClientFromEnv(), { leadId: id, userId: user.id });
