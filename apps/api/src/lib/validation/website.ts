...
import { dentalContentSchema, type DentalContent } from "@pitchmyweb/templates";
import { GENERIC_TEMPLATE_CODES } from "../websites/template-registry";
...
export const templateContentSchema = z
  .object({
    ...
    template: z.enum(GENERIC_TEMPLATE_CODES),
...
// Every content contract a WebsiteProject can hold: the generic one above,
// or a typed template contract from @pitchmyweb/templates (validated there,
// with the same no-markup rule).
export const websiteContentSchema = z.union([templateContentSchema, dentalContentSchema]);

export type WebsiteContent = TemplateContent | DentalContent;
...
export const websiteProjectCreateSchema = z
  .object({
    ...
    contentJSON: websiteContentSchema,
