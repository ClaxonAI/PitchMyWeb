...
describe("analyzeLead — successful analysis", () => {
  it("persists a valid AI response, transitions the lead, and records LEAD_ANALYZED (tests 1, 17, 18)", async () => {
    ...
    const result = await analyzeLead(prisma, ollama, { leadId: lead.id, userId: user.id });
  ...
  it("persists model name, prompt version, latency, and SUCCESS status (tests 13, 14, 15, 16)", async () => {
    ...
    await analyzeLead(prisma, ollama, { leadId: lead.id, userId: user.id });
    ...
    expect(analysis?.modelName).toBe("llama3-test");
...
describe("analyzeLead — repair retry behavior", () => {
  it("repairs malformed JSON on the second attempt and succeeds (tests 2, 4, 21)", async () => {
    ...
    const ollama = new ScriptedOllamaClient([{ text: "this is not json at all {" }, { text: validResponseText() }]);

    const result = await analyzeLead(prisma, ollama, { leadId: lead.id, userId: user.id });

    expect(result.lead.status).toBe("ANALYZED");
    expect(result.repairUsed).toBe(true);
    expect(ollama.callCount).toBe(2); // exactly one repair attempt, not more
  });
  ...
  it("repairs a schema-invalid response (arbitrary recommendedService) on the second attempt (tests 3, 4, 11, 21)", async () => {
    ...
    const ollama = new ScriptedOllamaClient([{ text: validResponseText({ recommendedService: "FAKE_SERVICE" as never }) }, { text: validResponseText() }]);
  ...
  it("repairs an invalid deal range (max < min) on the second attempt (test 12)", async () => {
    ...
    const ollama = new ScriptedOllamaClient([{ text: validResponseText({ estimatedDealMin: 30000, estimatedDealMax: 5000 }) }, { text: validResponseText() }]);
  ...
  it("fails permanently when both the original and repaired responses are invalid, without exceeding one repair (test 5, 21)", async () => {
    ...
    const ollama = new ScriptedOllamaClient([{ text: "not json" }, { text: "still not json" }]);

    await expect(analyzeLead(prisma, ollama, { leadId: lead.id, userId: user.id })).rejects.toThrow(AiAnalysisFailedError);
    expect(ollama.callCount).toBe(2); // exactly two attempts total, never a third
  });
});

describe("analyzeLead — failure isolation (tests 6, 8, 19, 20, 23)", () => {
  it("treats an Ollama timeout/connection failure as a failed attempt eligible for the one repair, and fails cleanly if repair also fails", async () => {
    const { user, lead } = await newAnalyzableLead("timeout-then-fail");
    const ollama = new ScriptedOllamaClient([{ error: new OllamaRequestError("Ollama request timed out after 30000ms") }, { error: new OllamaRequestError("connection refused") }]);
  ...
  it("a timeout on the very first attempt still gets exactly one repair chance before failing", async () => {
    ...
    const ollama = new ScriptedOllamaClient([{ error: new OllamaRequestError("timed out") }, { text: validResponseText() }]);
  ...
  it("never exposes the raw Ollama error message to the thrown error (section 19)", async () => {
    const { user, lead } = await newAnalyzableLead("sanitized-error");
    const ollama = new ScriptedOllamaClient([{ error: new OllamaRequestError("connect ECONNREFUSED 10.0.0.5:11434 apikey=sk-secret") }, { error: new OllamaRequestError("connect ECONNREFUSED 10.0.0.5:11434 apikey=sk-secret") }]);

    const error = await analyzeLead(prisma, ollama, { leadId: lead.id, userId: user.id }).catch((e) => e);
...
describe("analyzeLead — authorization and lifecycle (tests 9, 22, 24)", () => {
  it("throws NotFoundError (no data leakage) when analyzing another user's lead", async () => {
    ...
    await expect(analyzeLead(prisma, ollama, { leadId: lead.id, userId: other.id })).rejects.toThrow(NotFoundError);
    expect(ollama.callCount).toBe(0); // ownership is checked before ever calling Ollama
  });

  it("rejects re-analyzing an already-ANALYZED lead before calling Ollama, and does not touch the existing valid analysis (test 22)", async () => {
    const { user, lead } = await newAnalyzableLead("no-reanalyze");
    const firstOllama = new ScriptedOllamaClient([{ text: validResponseText() }]);
    const first = await analyzeLead(prisma, firstOllama, { leadId: lead.id, userId: user.id });
    ...
    // rejects this before any Ollama call.
    const secondOllama = new ScriptedOllamaClient([{ text: "garbage" }, { text: "still garbage" }]);
    await expect(analyzeLead(prisma, secondOllama, { leadId: lead.id, userId: user.id })).rejects.toThrow(InvalidLeadTransitionError);
    expect(secondOllama.callCount).toBe(0);
  ...
  it("permits retrying after a full AI failure, since the lead remains NEW (test 24)", async () => {
    ...
    const failingOllama = new ScriptedOllamaClient([{ text: "bad" }, { text: "still bad" }]);
    await expect(analyzeLead(prisma, failingOllama, { leadId: lead.id, userId: user.id })).rejects.toThrow(AiAnalysisFailedError);

    const stillNew = await prisma.lead.findUnique({ where: { id: lead.id } });
    expect(stillNew?.status).toBe("NEW");

    const succeedingOllama = new ScriptedOllamaClient([{ text: validResponseText() }]);
    const retried = await analyzeLead(prisma, succeedingOllama, { leadId: lead.id, userId: user.id });
