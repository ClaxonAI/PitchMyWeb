...
async function newAnalyzedLead(prefix: string, businessOverrides: Partial<BusinessProviderInput> = {}) {
  ...
  const providerInput: BusinessProviderInput = {
    // Phase 2 dedup engine: unique name/phone (not shared literals) so this
    // business doesn't tier-2/tier-4 match another concurrently-running
    // test file's identically-fixtured business. uniqueBusinessName keeps
    // the "Lakshmi Dental Care" prefix, so .toContain() assertions on the
    // generated pitch text below still pass.
...
describe("generatePitch — successful generation (tests 1, 2, 4, 5, 6, 7, 8, 9, 10, 13, 16)", () => {
  it("persists a valid pitch, records PITCH_GENERATED, and never transitions the lead to PITCHED", async () => {
    ...
    const result = await generatePitch(prisma, ai, { leadId: lead.id, userId: user.id });
  ...
  it("works when no website/demo project exists for the lead (test 13)", async () => {
    ...
    const result = await generatePitch(prisma, ai, { leadId: lead.id, userId: user.id });
    expect(result.pitch.status).toBe("GENERATED");
    expect(ai.prompts[0]).toContain("DEMO WEBSITE (only mention this if it is not null; never invent a demo URL):\nnull");
  });
  ...
  it("includes the current website/demo content when one exists (test 12)", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: validResponseText() }]);
    await generatePitch(prisma, ai, { leadId: lead.id, userId: user.id });

    expect(ai.prompts[0]).toContain("clinic-modern");
    expect(ai.prompts[0]).toContain(project.demoUrl!);
  });
  ...
  it("uses the lead's already-configured recommendedService pricing in the prompt (test 16)", async () => {
    ...
    expect(ai.prompts[0]).toContain('"recommendedService": "WEBSITE"');
    expect(ai.prompts[0]).toContain('"priceMin": 15000');
    expect(ai.prompts[0]).toContain('"priceMax": 25000');
  });
});
...
describe("generatePitch — repair retry and rejection (tests 3, 14, 21-style single retry)", () => {
  it("repairs malformed JSON on the second attempt and succeeds", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: "not json at all {" }, { text: validResponseText() }]);
  ...
  it("repairs a schema-invalid response (extra unexpected field) on the second attempt", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: JSON.stringify({ message: "Hi there", recommendedService: "WEBSITE" }) }, { text: validResponseText() }]);
  ...
  it("rejects a response containing a fabricated phone number, repairs, and succeeds with a clean message (test 14)", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: validResponseText("Call us right now at +91-9999999999 for a special deal!") }, { text: validResponseText() }]);
  ...
  it("fails permanently (and persists nothing) when both attempts contain fabricated facts", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: badText }, { text: badText }]);

    await expect(generatePitch(prisma, ai, { leadId: lead.id, userId: user.id })).rejects.toThrow(AiPitchFailedError);
    expect(ai.callCount).toBe(2); // exactly one repair, never a third attempt
  ...
  it("fails permanently on a model timeout/connection failure on both attempts, without corrupting the lead", async () => {
    const { user, lead } = await newAnalyzedLead("timeout");
    const ai = new ScriptedAiClient([{ error: new AiRequestError("timed out") }, { error: new AiRequestError("connection refused") }]);
  ...
  it("does not overwrite an existing valid pitch when a later generation attempt fails (test: failed pitch does not mutate lead)", async () => {
    ...
    const goodAi = new ScriptedAiClient([{ text: validResponseText("Original good pitch.") }]);
    const first = await generatePitch(prisma, goodAi, { leadId: lead.id, userId: user.id });

    const badAi = new ScriptedAiClient([{ text: "garbage" }, { text: "still garbage" }]);
    await expect(generatePitch(prisma, badAi, { leadId: lead.id, userId: user.id })).rejects.toThrow(AiPitchFailedError);
...
describe("generatePitch — authorization and preconditions (tests 11, 15)", () => {
  it("rejects generation for another user's lead, never calling the model", async () => {
    const { lead } = await newAnalyzedLead("owner-a");
    const other = await createTestUser("owner-b");
    createdUserIds.push(other.id);
    const ai = new ScriptedAiClient([{ text: validResponseText() }]);

    await expect(generatePitch(prisma, ai, { leadId: lead.id, userId: other.id })).rejects.toThrow(NotFoundError);
  ...
  it("rejects generation when the lead has no recommendedService yet (not analyzed)", async () => {
    ...
    const ai = new ScriptedAiClient([{ text: validResponseText() }]);
    await expect(generatePitch(prisma, ai, { leadId: lead.id, userId: user.id })).rejects.toThrow(ConflictError);
    expect(ai.callCount).toBe(0);
  });
