...
import { n8nWebhookEnvelopeSchema } from "./n8n-webhook";
...
describe("settingsUpsertSchema", () => {
  ...
  it("rejects a secret-looking key", () => {
    expect(settingsUpsertSchema.safeParse({ key: "N8N_WEBHOOK_SECRET", value: "x" }).success).toBe(false);
...
describe("n8nWebhookEnvelopeSchema", () => {
  it("rejects an unrecognized event type", () => {
    const result = n8nWebhookEnvelopeSchema.safeParse({ event: "SOMETHING_UNKNOWN", idempotencyKey: "abc" });
    expect(result.success).toBe(false);
  });

  it("rejects a missing idempotencyKey", () => {
    const result = n8nWebhookEnvelopeSchema.safeParse({ event: "CAMPAIGN_EXECUTION_STARTED" });
    expect(result.success).toBe(false);
  });
});
