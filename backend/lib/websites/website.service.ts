...
import type { Prisma, PrismaClient, WebsiteProject, WebsiteStatus, WebsiteVersion } from "../../generated/prisma/client";
...
import type { TemplateContent, WebsiteDesignJson, WebsiteListQuery } from "../validation/website";
...
function buildPublishedUrl(slug: string): string {
  return `${baseAppUrl()}/site/${slug}`;
}
 ...
 */
export async function publishWebsiteProject(db: PrismaClient, userId: string, projectId: string): Promise<WebsiteProject> {
  ...
  return db.$transaction(async (tx) => {
    const published = await tx.websiteProject.update({
      where: { id: projectId },
      data: { status: "PUBLISHED", publishedUrl: buildPublishedUrl(project.slug) },
    });
