import type { Business, Lead, Pitch, Prisma, PrismaClient, ServiceCode } from "../../generated/prisma/client";
import { normalizeBusinessInput } from "../business/normalize";
import { upsertCanonicalBusiness } from "../business/dedupe";
...
export async function ingestBusinessAsLead(db: PrismaClient, input: IngestBusinessAsLeadInput): Promise<IngestBusinessAsLeadResult> {
  ...
  // frontend request or an n8n webhook body — they must be validated with
  // Zod before anything downstream trusts their shape. This schema existed
  ...
  return db.$transaction(async (tx) => {
    const business = await upsertCanonicalBusiness(tx, normalized);
