...
let providerSeq = 0;
async function newLead(prefix: string, phone: string | null = "+91-9800000001") {
...
describe("generateWhatsAppAction", () => {
  it("generates a WhatsApp URL using only the verified phone and the existing pitch content (tests 17, 18)", async () => {
    const { user, lead } = await newLead("generate");
    ...
    expect(result.whatsappUrl.startsWith("https://wa.me/919800000001?text=")).toBe(true);
