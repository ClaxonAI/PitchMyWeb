import type { Campaign, CampaignStatus, PrismaClient } from "../../generated/prisma/client";
import { ConflictError, InvalidCampaignTransitionError, NotFoundError } from "../errors";
import type { CampaignCreateInput, CampaignListQuery, CampaignUpdateInput } from "../validation/campaign";
...
// n8n-triggered orchestration (n8n remains the orchestration layer).
...
export async function createCampaign(db: PrismaClient, userId: string, input: CampaignCreateInput): Promise<Campaign> {
 ...
 * *preparation* for execution only — it does not call n8n or perform the
 * long-running lead-discovery workflow itself (n8n remains the
 * orchestration layer per the architecture rule).
 *
