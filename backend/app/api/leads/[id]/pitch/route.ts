...
// check) -> path validation -> pitch service -> (Ollama client -> Zod
...
// in this file.
//
// `ollama` is injectable (defaults to the real env-configured client, the
...
// a real Ollama server. Resolved lazily inside the body (after
...
export async function handleGeneratePitch(db: PrismaClient, request: NextRequest, params: { id: string }, ollama?: OllamaClient): Promise<NextResponse> {
  ...
  // before the expensive Ollama call — same pattern as
  // POST /api/leads/:id/analyze and login's rate limit.
  enforceRateLimit(`pitch:user:${user.id}`, 20, WINDOW_MS);
